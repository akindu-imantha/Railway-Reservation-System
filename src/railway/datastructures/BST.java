...
// Binary search tree for reservations, ordered by reservation ID.
// Smaller IDs go left; larger IDs go right.
public class BST {
    // Internal tree item holding one reservation and two child links.
    ...
    private TreeNode insert(TreeNode node, Reservation reservation) {
        ...
        // Follow the left or right branch based on the reservation ID.
    ...
    private TreeNode delete(TreeNode node, int reservationId) {
        ...
        if (reservationId < node.reservation.getReservationId()) {
            ...
            // A node with two children is replaced by the smallest node on its right.
    ...
    public Reservation search(int reservationId) {
        ...
        // Move down only one branch at each step instead of checking every reservation.
